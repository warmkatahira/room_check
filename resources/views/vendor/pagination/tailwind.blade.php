            ...
            @for ($i = $start_page; $i <= $end_page; $i++)
                @if ($i == $paginator->currentPage())
                    <span aria-current="page">
                        <span class="bg-theme-sub relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-600 border border-gray-300 cursor-default leading-5">{{ $i }}</span>
                    </span>
                @else
