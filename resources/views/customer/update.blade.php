...
<x-app-layout>
    ...
    <form method="POST" action="{{ route('customer.update') }}" id="customer_update_form" class="grid grid-cols-12 m-0">
        ...
        <x-master.input type="text" name="customer_category" label="荷主分類" :db="$customer->customer_category" />
        <x-master.select name="base_id" label="拠点名" :forValue="$bases" forText="base_name" :db="$customer->base_id" />
